...
const
  ...
  MINIBROWSER_HOMEPAGE = 'https://wenku.baidu.com/';
...
type
  TMiniBrowserFrm = class(TForm)
    ...
    procedure mnuCopyWenkuClick(Sender: TObject);
...
type
  TTextRec = class
    ...
    LineHeight: Integer;
    Text: string;
    function CompareTo(R: TTextRec): Integer;
  end;

{ TTextRec }
function TTextRec.CompareTo(R: TTextRec): Integer;
begin
  if Self.Y > R.Y then
    Result := 1
  else if Self.Y = R.Y then
  begin
    if Self.X > R.X then
      Result := 1
    else if Self.X = R.X then
      Result := 0
    else
      Result := -1;
  end
...
function GetStyle(const key: string; const styles: TArray<TArray<string>>): string;
var
  I: Integer;
begin
...
function GetIntStyle(const key: string; const styles: TArray<TArray<string>>): Integer;
var
  s, v: string;
  I: Integer;
begin
  Result := 0;
  s := GetStyle(key, styles);
  v := '';
...
function ParseStyles(const style: string): TArray<TArray<string>>;
var
  sa, sa2: TArray<string>;
  I: Integer;
begin
  if style = '' then
...
procedure TMiniBrowserFrm.Chromium1BeforePopup(Sender: TObject;
...
procedure TMiniBrowserFrm.mnuCopyWenkuClick(Sender: TObject);
begin
  Chromium1.browser.MainFrame.GetSourceProc(
    procedure(const str: ustring)
    var
      LHtml: IHtmlElement;
      texts: IHtmlElementList;
      s, a: string;
      s2: AnsiString;
      sl: TStrings;
      I, J: Integer;
      styles: TArray<TArray<string>>;
      sa: TArray<string>;
      R: TTextRec;
      TextList: TList<TTextRec>;
      x, y, lh, totalLineHeight: Integer;
      Found: Boolean;
    begin
      LHtml := ParserHTML(str);
      texts := LHtml.Find('p.reader-word-layer');
      if texts.Count > 0 then
      begin
        totalLineHeight := 0;
        TextList := TObjectList<TTextRec>.Create;
        try
          ...
          begin
            ...
            if s <> '' then
            begin
              styles := ParseStyles(texts[I].Attributes['style']);
              x := GetIntStyle('left', styles);
              y := GetIntStyle('top', styles);
              lh := GetIntStyle('line-height', styles);
              Inc(totalLineHeight, lh);
              R := TTextRec.Create;
              R.X := x;
              R.Y := y;
              R.LineHeight := lh;
          ...
          begin
            ...
            try
              lh := totalLineHeight div TextList.Count;
              s := '';
              y := TextList[0].Y;
              lh := TextList[0].LineHeight;
              for I := 0 to TextList.Count-1 do
              begin
                if Abs(TextList[I].Y - y) < lh then
                begin
                  s := s + TextList[I].Text;
                end
                else
            ...
            ShowMessage('copy text to clipboard!');
          end
          else
          begin
            ShowMessage('text all empty!');
          end;
        finally
          TextList.Free;
        end;
...
begin
  ...
  if (title <> '') then
    caption := 'WenkuBrowser - ' + title
   else
    caption := 'WenkuBrowser';
end;
...
procedure TMiniBrowserFrm.URLCbxKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    GoBtnClick(Sender);
end;
